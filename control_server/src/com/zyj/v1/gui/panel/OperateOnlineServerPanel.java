...
public class OperateOnlineServerPanel extends JPanel {
	/**
	 * 
	 */
	private static final long serialVersionUID = 4775833491653129128L;
	 ...
	 * Create the panel.
	 */
	public OperateOnlineServerPanel() {
...
//		setthis(this);
		...
		button_start.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (select_servers.size() == 0) {
					JOptionPane.showMessageDialog(OperateOnlineServerPanel.this, "开服失败，未选择服务器！");
					return;
				}
		...
		button_stop.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (select_servers.size() == 0) {
					JOptionPane.showMessageDialog(OperateOnlineServerPanel.this, "停服失败，未选择服务器！");
					return;
				}
				operateType = OperateType.SERVER_MAINTAIN;
				clearResultDatas();
		...
		button_add_one.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (objects == null || objects.length == 0) {
					JOptionPane.showMessageDialog(OperateOnlineServerPanel.this, "未选择服务器！");
					return;
				}
...
//						JFileChooser.FILES_AND_DIRECTORIES);
			...
			*/
				...
				int result = chooser.showOpenDialog(SSHMain.mainFrame);
		...
		button_group_delete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (!radiobutton_group.isSelected()) {
					JOptionPane.showMessageDialog(OperateOnlineServerPanel.this, "请先选择 分组 选项！");
					return;
				}
				int index = jlistOperates.getSelectedIndex();
				if (index < 0) {
					JOptionPane.showMessageDialog(OperateOnlineServerPanel.this, "请先选择要删除的服务器组！");
					return;
				}
				if (index == 0) {
					JOptionPane.showMessageDialog(OperateOnlineServerPanel.this, "所有服务器组不可删除！");
					return;
				}
		...
		button_group_modify.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (obj == null) {
					JOptionPane.showMessageDialog(OperateOnlineServerPanel.this, "请先选择要修改的服务器组！");
					return;
				}
		...
		GroupLayout gl_contentPane = new GroupLayout(OperateOnlineServerPanel.this);
			...
			gl_contentPane.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_contentPane.createSequentialGroup()
					...
					.addGroup(gl_contentPane.createParallelGroup(Alignment.LEADING)
						...
						.addComponent(scrollPane_1, GroupLayout.PREFERRED_SIZE, 601, GroupLayout.PREFERRED_SIZE))
...
//						JOptionPane.showMessageDialog(contentPane, "未选择列表行！");	
	...
	public void selectUploadFiles(File[] selectedFiles) {
		...
		//不知满足则结束
		if(!upload_files.setNeedMappingFiles(OnlineServerManager.APP_JAR,SSHMain.mainFrame.getPanel_operate_online_server(),
				sshMgr.getMainServer(),selectedFiles)) {
			return;
		}
