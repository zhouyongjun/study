...
import com.zhou.util.sfs.data.SFSArrayLite;
import com.zhou.util.sfs.data.SFSDataType;
import com.zhou.util.sfs.data.SFSDataWrapper;
import com.zhou.util.sfs.data.SFSObject;
import com.zhou.util.sfs.data.SFSObjectLite;
import com.zhou.util.sfs.data.exceptions.SFSCodecException;
import com.zhou.util.sfs.data.exceptions.SFSRuntimeException;
...
public class DefaultSFSDataSerializer implements ISFSDataSerializer {
	...
	public ISFSArray binary2array(byte[] data) {
		if (data.length < 3) {
			throw new IllegalStateException(
					"Can't decode an SFSArray. Byte data is insufficient. Size: "
							+ data.length + " bytes");
		}
	...
	private ISFSArray decodeSFSArray(ByteBuffer buffer) {
		...
		if (headerBuffer != SFSDataType.SFS_ARRAY.getTypeID()) {
			throw new IllegalStateException("Invalid SFSDataType. Expected: "
					+ SFSDataType.SFS_ARRAY.getTypeID() + ", found: "
					+ headerBuffer);
		}
		...
		if (size < 0) {
			throw new IllegalStateException(
					"Can't decode SFSArray. Size is negative = " + size);
		}
		...
		try {
			for (int i = 0; i < size; i++) {
				...
				if (decodedObject != null)
					...
					throw new IllegalStateException(
							"Could not decode SFSArray item at index: " + i);
				}
			}
	...
	public ISFSObject binary2object(byte[] data) {
		if (data.length < 3) {
			throw new IllegalStateException(
					"Can't decode an SFSObject. Byte data is insufficient. Size: "
							+ data.length + " bytes");
		}
		ByteBuffer buffer = ByteBuffer.allocate(data.length);
		buffer.put(data);
	...
	private ISFSObject decodeSFSObject(ByteBuffer buffer) {
		...
		if (headerBuffer != SFSDataType.SFS_OBJECT.getTypeID()) {
			throw new IllegalStateException("Invalid SFSDataType. Expected: "
					+ SFSDataType.SFS_OBJECT.getTypeID() + ", found: "
					+ headerBuffer);
		}
		...
		if (size < 0) {
			throw new IllegalStateException(
					"Can't decode SFSObject. Size is negative = " + size);
		}
		...
		try {
			for (int i = 0; i < size; i++) {
				...
				if ((keySize < 0) || (keySize > 255)) {
					throw new IllegalStateException(
							"Invalid SFSObject key length. Found = " + keySize);
				}
				...
				if (decodedObject != null)
					...
					throw new IllegalStateException(
							"Could not decode value for key: " + keyData);
				}
			}
	...
	public ISFSArray json2array(String jsonStr) {
		if (jsonStr.length() < 2) {
			throw new IllegalStateException(
					"Can't decode SFSObject. JSON String is too short. Len: "
							+ jsonStr.length());
		}
	...
	private ISFSArray decodeSFSArray(JSONArray jsa) {
		...
		for (Iterator<Object> iter = jsa.iterator(); iter.hasNext();) {
			...
			if (decodedObject != null)
				...
				throw new IllegalStateException(
						"(json2sfarray) Could not decode value for object: "
								+ value);
			}
		}
		return sfsArray;
	...
	private ISFSObject decodeSFSObject(JSONObject jso) {
		...
		for (Iterator localIterator = jso.keySet().iterator(); localIterator
				.hasNext();) {
			...
			if (decodedObject != null)
				...
				throw new IllegalStateException(
						"(json2sfsobj) Could not decode value for key: " + key);
			}
		}
		return sfsObject;
	}
	...
	private SFSDataWrapper decodeJsonObject(Object o) {
		...
		if ((o instanceof JSONObject)) {
			...
			return new SFSDataWrapper(SFSDataType.SFS_OBJECT,
					decodeSFSObject(jso));
		}

		if ((o instanceof JSONArray)) {
			return new SFSDataWrapper(SFSDataType.SFS_ARRAY,
					decodeSFSArray((JSONArray) o));
		}
	...
	private byte[] getBlobData(String colName, InputStream stream) {
		...
		try {
			...
			this.logger
					.warn("SFSObject serialize error. Failed reading BLOB data for column: "
							+ colName);
		} finally {
	...
	private SFSDataWrapper decodeObject(ByteBuffer buffer)
		...
		if (headerByte == SFSDataType.NULL.getTypeID()) {
			...
			decodedObject = new SFSDataWrapper(SFSDataType.SFS_ARRAY,
	...
	private ByteBuffer encodeObject(ByteBuffer buffer, SFSDataType typeId,
			Object object) {
		switch (typeId) {
		case BOOL:
		...
		default:
			throw new IllegalArgumentException(
					"Unrecognized type in SFSObject serialization: " + typeId);
		}
	...
	private SFSDataWrapper binDecode_BOOL(ByteBuffer buffer)
			throws SFSCodecException {
		...
		if (boolByte == 0)
			...
			throw new SFSCodecException(
					"Error decoding Bool type. Illegal value: " + bool);
		}
	...
	private SFSDataWrapper binDecode_DOUBLE(ByteBuffer buffer) {
		...
		return new SFSDataWrapper(SFSDataType.DOUBLE,
	...
	private SFSDataWrapper binDecode_UTF_STRING(ByteBuffer buffer)
		...
		if (strLen < 0) {
			throw new SFSCodecException(
					"Error decoding UtfString. Negative size: " + strLen);
		}
	...
	private SFSDataWrapper binDecode_BOOL_ARRAY(ByteBuffer buffer)
			throws SFSCodecException {
		...
		for (int j = 0; j < arraySize; j++) {
			...
			if (boolData == 0)
				...
				throw new SFSCodecException(
						"Error decoding BoolArray. Invalid bool value: "
								+ boolData);
			}
		}
	...
	private SFSDataWrapper binDecode_BYTE_ARRAY(ByteBuffer buffer)
		...
		if (arraySize < 0) {
			throw new SFSCodecException(
					"Error decoding typed array size. Negative size: "
							+ arraySize);
		}
	...
	private SFSDataWrapper binDecode_SHORT_ARRAY(ByteBuffer buffer)
			throws SFSCodecException {
		short arraySize = getTypeArraySize(buffer);
		List array = new ArrayList();
	...
	private SFSDataWrapper binDecode_INT_ARRAY(ByteBuffer buffer)
			throws SFSCodecException {
		short arraySize = getTypeArraySize(buffer);
		List array = new ArrayList();
	...
	private SFSDataWrapper binDecode_LONG_ARRAY(ByteBuffer buffer)
			throws SFSCodecException {
		short arraySize = getTypeArraySize(buffer);
		List array = new ArrayList();
	...
	private SFSDataWrapper binDecode_FLOAT_ARRAY(ByteBuffer buffer)
			throws SFSCodecException {
		short arraySize = getTypeArraySize(buffer);
	...
	private SFSDataWrapper binDecode_DOUBLE_ARRAY(ByteBuffer buffer)
			throws SFSCodecException {
		short arraySize = getTypeArraySize(buffer);
	...
	private SFSDataWrapper binDecode_UTF_STRING_ARRAY(ByteBuffer buffer)
			throws SFSCodecException {
		short arraySize = getTypeArraySize(buffer);
		...
		for (int j = 0; j < arraySize; j++) {
			...
			if (strLen < 0) {
				throw new SFSCodecException(
						"Error decoding UtfStringArray element. Element has negative size: "
								+ strLen);
			}
	...
	private ByteBuffer binEncode_BOOL_ARRAY(ByteBuffer buffer,
	...
	private ByteBuffer binEncode_SHORT_ARRAY(ByteBuffer buffer,
	...
	private ByteBuffer binEncode_INT_ARRAY(ByteBuffer buffer,
	...
	private ByteBuffer binEncode_LONG_ARRAY(ByteBuffer buffer,
	...
	private ByteBuffer binEncode_FLOAT_ARRAY(ByteBuffer buffer,
	...
	private ByteBuffer binEncode_DOUBLE_ARRAY(ByteBuffer buffer,
	...
	private ByteBuffer binEncode_UTF_STRING_ARRAY(ByteBuffer buffer,
		...
		for (Field field : pojoClazz.getDeclaredFields()) {
			try {
				...
				if (Modifier.isPublic(modifiers)) {
					...
					fieldValue = readValueFromGetter(fieldName, field.getType()
							.getSimpleName(), pojo);
				}
				...
				this.logger
						.info("-- No public getter -- Serializer skipping private field: "
								+ field.getName()
								+ ", from class: "
								+ pojoClazz);
				err.printStackTrace();
	...
	private Object readValueFromGetter(String fieldName, String type,
	...
	private SFSDataWrapper wrapPojoField(Object value) {
		...
		if ((value instanceof Boolean)) {
			...
			wrapper = new SFSDataWrapper(SFSDataType.SFS_ARRAY,
					unrollArray((Object[]) value));
		} else if ((value instanceof Collection)) {
			wrapper = new SFSDataWrapper(SFSDataType.SFS_ARRAY,
					unrollCollection((Collection) value));
		} else if ((value instanceof Map)) {
			wrapper = new SFSDataWrapper(SFSDataType.SFS_OBJECT,
					unrollMap((Map) value));
	...
	public Object sfs2pojo(ISFSObject sfsObj) {
		...
		if ((!sfsObj.containsKey("$C")) && (!sfsObj.containsKey("$F"))) {
			throw new SFSRuntimeException(
					"The SFSObject passed does not represent any serialized class.");
		}
		try {
	...
	private void setObjectField(Object pojo, String fieldName, Object fieldValue)
		...
		if (isArray) {
			if (!(fieldValue instanceof Collection)) {
				throw new SFSRuntimeException(
						"Problem during SFSObject => POJO conversion. Found array field in POJO: "
			...
			} else if ((fieldClass.equals("Set"))
					|| (fieldClass.equals("HashSet"))) {
			...
			} else if ((fieldClass.equals("Queue"))
					|| (fieldClass.equals("PriorityQueue"))) {
				fieldValue = new PriorityQueue(collection);
			} else if ((fieldClass.equals("BlockingQueue"))
					|| (fieldClass.equals("LinkedBlockingQueue"))) {
			...
			} else if ((fieldClass.equals("Deque"))
					|| (fieldClass.equals("ArrayDeque"))) {
	...
	private void writeValueFromSetter(Field field, Object pojo,
			Object fieldValue) throws Exception {
		String setterName = "set" + StringUtils.capitalize(field.getName());
		try {
			Method setterMethod = pojo.getClass().getMethod(setterName,
					new Class[] { field.getType() });
			setterMethod.invoke(pojo, new Object[] { fieldValue });
		} catch (NoSuchMethodException e) {
			this.logger
					.info("-- No public setter -- Serializer skipping private field: "
							+ field.getName()
							+ ", from class: "
