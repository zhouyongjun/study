...
import com.zhou.core.astar.test.map.AStar_V4.Node;
import com.zhou.core.astar.test.map.AStar_V4.RectCell;
public class AStarFrame_V4 extends JFrame {
	static AStarFrame_V4 frame ;
	/**
	 * 
	 */
...
//					frame.test();
	...
	static class MyRunnable implements Runnable {
		AStarFrame_V4 frame;
		MyRunnable(AStarFrame_V4 frame) {
			this.frame = frame;
		}
		@Override
		public void run() {
...
//		this.getContentPane().setBounds(100, 100, 1024, 1024);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (isRuned)
				{
					for (RectCell cell : aStar.getCells().values())
					{
						cell.resest();
					}
					for (Entry<Integer,Integer> entry : aStar.getValues().entrySet())
					{
						aStar.getValues().put(entry.getKey(), Integer.MAX_VALUE);
					}
					repaint();
				}
				test();
